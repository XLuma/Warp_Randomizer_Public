...
from nds.gen4 import PlatinumWarpMapInfo as info, PlatinumWarpMapInfo
...
class PlatinumRandomizerFunctions(SructureDefinitions.GenRandomizerFunctions):
    def __init__(self):
        self.maps = []
        self.map_name_to_id = dict()
        self.start_map = "Map_Twinleaf_Town_00"
        ...
        for map_name in total_maps:
            if map_name not in in_map:
                if 'Frontier' not in map_name and 'Unused' not in map_name and 'Dummy' not in map_name and \
                        'Battle_Factory' not in map_name and 'Battle_Hall' not in map_name and 'Battle_Castle' \
                        not in map_name and 'Battle_Arcade' not in map_name and 'Battle_Tower' not in map_name and \
                        'Registeel_Empty_Cave' not in map_name and 'Regirock_Empty_Cave' not in map_name and \
                        'Regice_Empty_Cave' not in map_name and 'Elevator' not in map_name:
    ...
    def is_not_needed_map_ok(self, map_name: str) -> bool:
        if 'Room' in map_name or 'Shop' in map_name:
    ...
    def insert_data_into_binary(self, map_name, warp):
        can_write = True
        for map_group in PlatinumWarpMapInfo.grouped_warps:
            if map_name in PlatinumWarpMapInfo.grouped_warps[map_group]['warps']:
                can_write = False
            elif warp.dest_map in PlatinumWarpMapInfo.grouped_warps[map_group]['warps']:
                can_write = False
    ...
    def fix_warp_groupings(self, map_warps):
        ...
        for group_name in PlatinumWarpMapInfo.grouped_warps:
            entry = PlatinumWarpMapInfo.grouped_warps[group_name]
            if 'header_dummy' in list(entry.keys()):
                ...
                with open(Utils.resource_path(
                        os.path.join('Resources', 'gen4', 'plat_fixes', 'groupings', resource_folder, 'script.scr')),
                        'rb') as f:
    ...
    def overwrite_other_data(self, map_warps):
        for group in PlatinumWarpMapInfo.other_overwrites:
            entry = PlatinumWarpMapInfo.other_overwrites[group]
            resource_folder = entry['resource_folder']

            print('Writing patches for: %s' % str(group))

            if group == 'Tornworld_return':
                map_name = 'Map_Distortion_World_00'
                script_id = entry['script_overwrite']
                with open(Utils.resource_path(
                        os.path.join('Resources', 'gen4', 'plat_fixes', 'overwrites', resource_folder,
    ...
    def write_rom(self, input_file: str, output_file: str, map_warps: dict):
        ...
        self.events_narc = ndspy.narc.NARC(rom.getFileByName('/fielddata/eventdata/zone_event.narc'))
        self.maps_narc = ndspy.narc.NARC(rom.getFileByName('/fielddata/land_data/land_data.narc'))
        self.matrix_narc = ndspy.narc.NARC(rom.getFileByName('/fielddata/mapmatrix/map_matrix.narc'))
        self.scripts_narc = ndspy.narc.NARC(rom.getFileByName('/fielddata/script/scr_seq.narc'))
        self.text_narc = ndspy.narc.NARC(rom.getFileByName('/msgdata/pl_msg.narc'))

        with open(Utils.resource_path(os.path.join('Resources', 'gen4', 'plat_fixes', 'groupings', 'dummy_event.evt')),
                  'rb') as f:
            data = bytearray(f.read())
        self.events_narc.files.append(data)

        with open(Utils.resource_path(os.path.join('Resources', 'gen4', 'plat_fixes', 'groupings', 'dummy_map.bin')),
                  'rb') as f:
            data = bytearray(f.read())
        ...
        with open(Utils.resource_path(os.path.join('Resources', 'gen4', 'plat_fixes', 'groupings', 'dummy_matrix.mtx')),
                  'rb') as f:
            data = bytearray(f.read())
        ...
        with open(Utils.resource_path(
                os.path.join('Resources', 'gen4', 'plat_fixes', 'groupings', 'dummy_level_script.scr')), 'rb') as f:
            data = bytearray(f.read())
        self.scripts_narc.files.append(data)
        ...
        for map_id in range(len(maps)):
            for row in range(32):
                for col in range(32):
                    ...
                    if perm == 0x6A or perm == 0x6B or perm == 0x5E or perm == 0x5F:
        ...
        rom.setFileByName('/fielddata/eventdata/zone_event.narc', self.events_narc.save())
        rom.setFileByName('fielddata/land_data/land_data.narc', self.maps_narc.save())
        rom.setFileByName('/fielddata/mapmatrix/map_matrix.narc', self.matrix_narc.save())
        rom.setFileByName('/fielddata/script/scr_seq.narc', self.scripts_narc.save())
        rom.setFileByName('/msgdata/pl_msg.narc', self.text_narc.save())
