...
class PlatinumReadWriteHandler:
    ...
    def build_lists(self, rom, headers):
        ...
        for y in range(len(matrix_narc.files)):
            headers_by_area_data.append([])
            for x in range(75):
                headers_by_area_data[y].append([])

        for y in range(len(headers_by_area_data)):
            for x in headers_by_matrix[y]:
        ...
        # PlatinumFileDataGenerator.write_json(headers_by_area_data, headers, matrices, events, rom)
