"""
White2ReadWriteHandler.py

Class to handle initial read/writes in development
...
from nds.gen5.dev import White2FileDataGenerator
from nds.gen5.FormatsGen5 import *
from nds.buffer import Buffer


class White2ReadWriteHandler:
    ...
    def build_lists(self, rom, headers):
        ...
        for y in range(len(matrix_narc.files)):  # one entry for each matrix
            ...
            for x in range(401):  # one entry for each area data value
        ...
        for y in range(len(headers_by_area_data)):  # iterates through for each matrix num
            for x in headers_by_matrix[y]:  # creates a structure readable as: headers_by_area_data[matrix id][area data ID] -> list of what headers use that area data ID
                headers_by_area_data[y][headers[x].texture_container_idx].append(x)
        ...
        # White2FileDataGenerator.write_json(headers_by_area_data, headers, matrices, events, rom)
        White2FileDataGenerator.fix_json(headers_by_area_data, headers, matrices, events, rom)
